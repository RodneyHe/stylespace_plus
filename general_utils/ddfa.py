...
u_base = torch.from_numpy(u_base).to(DEVICE)
w_shp = torch.from_numpy(w_shp).to(DEVICE)
w_shp_base = torch.from_numpy(w_shp_base).to(DEVICE)
w_exp = torch.from_numpy(w_exp).to(DEVICE)
w_exp_base = torch.from_numpy(w_exp_base).to(DEVICE)
...
def reconstruct_vertex(param, whitening=True, dense=False, transform=True):
    ...
    """
    if len(param) == 12:
        param = torch.concat((param, [0] * 50))
    if whitening:
        if len(param) == 62:
            param = param * param_std + param_mean
        else:
            param = torch.concat((param[:11], [0], param[11:]))
            param = param * param_std + param_mean

    p, offset, alpha_shp, alpha_exp = _parse_param(param)

    if dense:
        tmp = (u_base + w_shp_base @ alpha_shp + w_exp_base @ alpha_exp).squeeze()
        vertex = p @ torch.concat((tmp[0::3][None, ...], tmp[1::3][None, ...], tmp[2::3][None, ...]), 0) + offset
